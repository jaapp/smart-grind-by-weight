...
**Display Settings:** Brightness sliders (normal + screensaver dim level), screensaver enable toggle, and a Preview button that shows the screensaver immediately (held until tapped, via `ScreenTimeoutController::start_screensaver_now()`). The screensaver variant is picked by dragging left/right on the screensaver itself (interactive tileview paging), persisted as `variant` in preferences namespace `screensaver` (int: 0=Wave, 1=Trains grouped, 2=Trains board; legacy `style`/`layout` keys are migrated as the default). Other keys: `enabled` (bool, default true). The screensaver appears after the 2-minute idle dim, dismisses on tap (swallowed) or scale activity, and never shows while grinding, during a manual run, or during OTA.
