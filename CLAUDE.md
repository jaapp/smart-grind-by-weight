...
- **ScreensaverOverlay**: Full-screen idle overlay on `lv_layer_top()` cycling through three variants by swiping left/right on the overlay itself (choice persisted): Wave (brown dot-matrix ripple via a custom draw callback), Trains grouped (upcoming NYC subway arrivals from the MTA gateway, one entry per tracked watch in stable gateway order - route bullet in authentic MTA proportions, stacked destination + station name, and below them a full-width row of arrival-time pills; trains already due are dropped as unreachable, watches with no upcoming arrivals hidden), and Trains board (flat time-sorted list with one row per train and a big countdown on the right, "Now" when due). Minutes count down locally between polls; connect/loading/unreachable states, a stale-data marker after 90s without a successful poll, and fallback to the error state after 5min - `NET_DATA_STALE_MS`/`NET_DATA_EXPIRED_MS`. A tap dismisses immediately; swiping up slides the overlay off the top to reveal the screen underneath; all touches are swallowed. Swiping up/down on the ready screen starts the screensaver with a matching slide-down entry (`ScreenTimeoutController::start_screensaver_now()`, held until tapped); the idle timeout shows it without animation
...
**Display Settings:** Brightness sliders (normal + screensaver dim level), screensaver enable toggle, and a Preview button that shows the screensaver immediately (held until tapped, via `ScreenTimeoutController::start_screensaver_now()`). The screensaver variant is picked by swiping left/right on the screensaver itself, persisted as `variant` in preferences namespace `screensaver` (int: 0=Wave, 1=Trains grouped, 2=Trains board; legacy `style`/`layout` keys are migrated as the default). Other keys: `enabled` (bool, default true). The screensaver appears after the 2-minute idle dim, dismisses on tap (swallowed) or scale activity, and never shows while grinding, during a manual run, or during OTA.
