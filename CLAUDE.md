...
- **Fast Mode**: Speed-over-accuracy toggle (default: disabled). Skips PRIME/PURGE phases, shortens tare (6 samples vs 18) and settling windows (200ms vs 500ms), widens tolerance to 0.1g, aims pulses at the exact target (normal mode aims at target minus tolerance), caps pulse corrections at 3. Constants in `grind_control.h` (`GRIND_FAST_*`)
