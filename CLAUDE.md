...
- **ScreensaverOverlay**: Full-screen idle overlay on `lv_layer_top()` with two styles selected in Display Settings: Wave (brown dot-matrix ripple via a custom draw callback) and Trains (upcoming NYC subway arrivals from the MTA gateway, with two selectable layouts: Grouped (default) shows one entry per tracked watch in stable gateway order - route bullet in authentic MTA proportions, stacked destination + station name, and below them a full-width row of arrival-time pills (trains already due are dropped as unreachable, watches with no upcoming arrivals hidden); Board shows a flat time-sorted list with one row per train and a big countdown on the right ("Now" when due). Minutes count down locally between polls; connect/loading/unreachable states, a stale-data marker after 90s without a successful poll, and fallback to the error state after 5min - `NET_DATA_STALE_MS`/`NET_DATA_EXPIRED_MS`). Any press dismisses and is swallowed. Swiping up/down on the ready screen starts the screensaver immediately (`ScreenTimeoutController::start_screensaver_now()`, held until touch)
...
**Display Settings:** Brightness sliders (normal + screensaver dim level), screensaver enable toggle, a Wave/Trains style radio, a Grouped/Board Trains-layout radio, and a Preview button that shows the screensaver immediately (held until touch, via `ScreenTimeoutController::start_screensaver_now()`). Preferences namespace `screensaver`: `enabled` (bool, default true), `style` (int: 0=Wave, 1=Trains, default 0), `layout` (int: 0=Grouped, 1=Board, default 0). The screensaver appears after the 2-minute idle dim, wakes on touch (swallowed) or scale activity, and never shows while grinding, during a manual run, or during OTA.
