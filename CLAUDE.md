...
- **ScreensaverOverlay**: Full-screen idle overlay on `lv_layer_top()`, built as a horizontal `lv_tileview` of three pages; tapping the leftmost/rightmost 48px of the screen (`kEdgeTapZonePx`) slides to the previous/next page (animated, clamped at the ends, tiles are not draggable), tapping anywhere else dismisses (both trains pages are pre-built so a slide never reveals a blank neighbor; the shown page is persisted): Wave (brown dot-matrix ripple via a custom draw callback), Trains grouped (upcoming NYC subway arrivals from the MTA gateway, one entry per tracked watch in stable gateway order - route bullet in authentic MTA proportions, stacked destination + station name, and below them a full-width row of arrival-time pills; watches with no upcoming arrivals hidden), and Trains board (flat time-sorted list with one row per train and a big countdown on the right); both trains pages stack their rows from the top of the screen (only the no-rows status message is centered), show trains arriving right now as 0m and only drop trains the local countdown has pushed past due. Rows beyond one screenful (4 watches grouped, 7 trains on the board, 6 when the stale marker needs the last slot) are split across fixed pages filled in order (4+1, not balanced, so a watch always lands on the same page); swiping up/down anywhere on the screensaver renders the next/previous page outright (no scroll animation, clamped at the ends, never auto-advances) and a dot column in a 14px right gutter marks the pages (only when there is more than one; `kPageDot*`, `THEME_COLOR_SCREENSAVER_PAGE_DOT`). Each trains view keeps its own page index, reset on show; when a poll drops the shown page it clamps to the last page that still exists (`resolve_trains_page()`). Watches with a walk-to-platform estimate (set in the gateway UI) flag hard catches by color: yellow = only reachable by rushing (walk covered in `NET_WALK_RUSH_PERCENT` = 75% of normal time), red = can't be caught; comfortable catches and watches with no estimate are unflagged. The grouped view rings the arrival pill in that color (every pill reserves the 2px ring width so flagged pills stay the same size); the board view, which has no pill, colors the countdown text and leaves it white otherwise (`arrival_catch_color()`/`apply_catch_ring()` in screensaver.cpp, `THEME_COLOR_SCREENSAVER_CATCH_*`). Minutes count down locally between polls; connect/loading/unreachable states, a stale-data marker after 90s without a successful poll, and fallback to the error state after 5min - `NET_DATA_STALE_MS`/`NET_DATA_EXPIRED_MS`. All touches are swallowed. Swiping up/down on the ready screen starts the screensaver immediately (`ScreenTimeoutController::start_screensaver_now()`, held until tapped)
- **TrainDataClient** (`src/network/`): WiFi lifecycle + gateway polling on a dedicated low-priority FreeRTOS task (core 1, 250ms tick; during OTA the radio is taken down via `pause_wifi()` before the task is suspended and brought back with `resume_wifi()` on abort/failure). Connects with NVS-stored credentials (`network` namespace: `ssid`, `pass`, `url`), polls `<url>/api/arrivals` every 30s only while the Trains screensaver is visible, parses with cJSON into a mutex-guarded snapshot. Provisioned via BLE debug opcodes 0x03 (set config) / 0x04 (status) from `tools/grinder.py wifi`
- **BLE OTA** (`src/bluetooth/`): `OTAHandler` receives a detools/heatshrink patch into the `patch` partition and applies it to the inactive app slot; `BluetoothManager` acknowledges END on the NimBLE thread and runs the apply from the bluetooth task (`finish_ota()`), notifying `BLE_OTA_SUCCESS` before restarting or `BLE_OTA_ERROR` + reason text on failure (UI then shows the Update Failed screen via `take_ota_failure()`). The OTA service exposes the running image's ELF SHA-256 (`BLE_OTA_FIRMWARE_ID_CHAR_UUID`) so `tools/ble/grinder-ble.py` can pick a matching delta base from `firmware_cache/` instead of trusting per-checkout build numbers
...
- **Automation**: Start on Cup and Return on Removal toggles (weight/time panes only, never Manual/Menu). Cup detection settles against a relaxed 0.05g tolerance (`USER_AUTO_GRIND_SETTLE_TOLERANCE_G`); grind tare/settling keeps the precision 0.010g tolerance. Return on Removal fires for any completed grind (SUCCESS/OVERSHOOT/MAX_PULSES) and for sessions ended by the negative-weight failsafe (`GrindSessionResult::CUP_REMOVED`, cup pulled mid-settle); other timeout/error screens still require acknowledgment
- **Fast Mode**: Speed-over-accuracy toggle (default: disabled). Skips PRIME/PURGE phases, shortens tare (6 samples vs 18) and settling windows (200ms vs 500ms), widens tolerance to 0.05g, stops the motor early by an extra dose-proportional margin (5% of target, clamped 0.2-0.6g) so the main grind lands under target instead of coasting past it, aims pulses at target minus half the tolerance to offset post-pulse dribble (normal mode aims at target minus tolerance), caps pulse corrections at 3, and shortens the Start-on-Cup settled check (200ms vs 500ms). Constants in `grind_control.h` (`GRIND_FAST_*`)
...
**Display Settings:** Brightness sliders (normal + screensaver dim level), screensaver enable toggle, and a Preview button that shows the screensaver immediately (held until tapped, via `ScreenTimeoutController::start_screensaver_now()`). The screensaver variant is picked by tapping the left/right edge of the screensaver itself (slides to the neighboring page), persisted as `variant` in preferences namespace `screensaver` (int: 0=Wave, 1=Trains grouped, 2=Trains board; legacy `style`/`layout` keys are migrated as the default). Other keys: `enabled` (bool, default true). The screensaver appears after the 2-minute idle dim, dismisses on a tap away from the edges (swallowed) or scale activity, and never shows while grinding, during a manual run, or during OTA.

**MTA Train Gateway (`gateway/`):** Dockerized FastAPI service that polls the MTA's public subway GTFS-realtime feeds and serves `GET /api/arrivals` (per watch: route, official color, friendly direction label, next arrival minutes, optional user-entered walk-to-platform minutes) for the Trains screensaver, plus a web UI on port 8600 for picking station/line/direction watches and entering each watch's 🚶 walk time (1-60 min, `PATCH /api/watches/{index}`, mirrored in the web UI's arrival colors). Auto-published to `ghcr.io/sebastienstdenis/mta-gateway` by `.github/workflows/gateway-image.yml` on pushes to main touching `gateway/`. See `gateway/README.md`.
