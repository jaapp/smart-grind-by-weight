...
- **ScreensaverOverlay**: Full-screen idle overlay on `lv_layer_top()` with two styles selected in Display Settings: Wave (brown dot-matrix ripple via a custom draw callback) and Trains (a time-sorted departure board of upcoming NYC subway arrivals from the MTA gateway: one row per arrival - route bullet in authentic MTA proportions, stacked destination + station name, minutes counting down locally between polls with "Now" when due; connect/loading/unreachable states, a stale-data marker after 90s without a successful poll, and fallback to the error state after 5min - `NET_DATA_STALE_MS`/`NET_DATA_EXPIRED_MS`). Any press dismisses and is swallowed. Swiping up/down on the ready screen starts the screensaver immediately (`ScreenTimeoutController::start_screensaver_now()`, held until touch)
