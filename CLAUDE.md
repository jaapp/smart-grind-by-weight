...
- **Fast Mode**: Speed-over-accuracy toggle (default: disabled). Skips PRIME/PURGE phases, shortens tare (6 samples vs 18) and settling windows (200ms vs 500ms), widens tolerance to 0.1g, stops the motor early by an extra dose-proportional margin (5% of target, clamped 0.2-1.0g) so the main grind lands under target instead of coasting past it, aims pulses at target minus half the tolerance to offset post-pulse dribble (normal mode aims at target minus tolerance), caps pulse corrections at 3, and shortens the Start-on-Cup settled check (200ms vs 500ms). Constants in `grind_control.h` (`GRIND_FAST_*`)
