...
void CalibratedTimeGrindStrategy::on_enter(const GrindSessionDescriptor&,
...
bool CalibratedTimeGrindStrategy::update(const GrindSessionDescriptor& session,
                                         GrindStrategyContext& context,
                                         const GrindLoopData& loop_data) {
    auto* controller = context.controller;
    if (!controller || session.mode != GrindMode::CALIBRATED_TIME) return false;
...
void CalibratedTimeGrindStrategy::on_exit(const GrindSessionDescriptor&,
                                          GrindStrategyContext& context) {
...
int CalibratedTimeGrindStrategy::progress_percent(const GrindSessionDescriptor& session,
                                                   const GrindController& controller) const {
    if (session.mode != GrindMode::CALIBRATED_TIME || session.target_time_ms == 0) return 0;
    if (controller.time_grind_start_ms == 0) return 0;

    unsigned long elapsed = millis() - controller.time_grind_start_ms;
    if (elapsed >= session.target_time_ms) return 100;
