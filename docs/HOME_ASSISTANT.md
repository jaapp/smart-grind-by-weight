...
The bridge is disabled by default so existing BLE-only builds keep the same
behavior unless Wi-Fi and MQTT details are provided.

## Enable At Build Time

Add a local PlatformIO environment with build flags for your network and broker:
...
build_flags =
    ...
    '-DHA_WIFI_SSID="YourWifi"'
    '-DHA_WIFI_PASSWORD="YourWifiPassword"'
