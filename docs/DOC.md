...
- **Fast Mode**: Trade accuracy for speed. Skips the purge/prime phase entirely, shortens the tare (6 samples instead of 18) and settling windows (200ms instead of 500ms), widens the target tolerance to 0.1g (a result within 0.1g of target ends the grind), aims correction pulses at the exact target instead of the conservative low edge, and allows at most 3 pulse corrections (instead of 10). Expect grinds to finish several seconds sooner, with more shot-to-shot variance and a higher chance of uncorrectable overshoot (default: disabled)
